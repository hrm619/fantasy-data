...
- **Play-caller beats OC title for the decay — weakly, on the cleaner metric**: backtested over the 2023->2024 and 2024->2025 transitions (2023-2026 have play_caller data). Where the two signals disagree, target-share volatility follows the play-caller, not the OC title (0.032 vs 0.024 abs Δ target share overall; 0.041 vs 0.032 among players with prior target_share >= 0.10). Directionally consistent and it validates the basis, but the discriminating sample is ~7 teams / ~17 contributor-events — a direction, not a settled result. `fpts_per_game` volatility pointed the other way, but it is dominated by TD/game-script/injury noise, so target_share (a purer role signal) is the metric to trust. Revisit after 2026 plays adds a third transition; only the 2019-2022 backfill (probe: expensive, missing-not-at-random) would settle it. Scratch analysis: `scratchpad/backtest_playcaller.py`.
