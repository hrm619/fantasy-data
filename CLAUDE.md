...
- **Divergence filters thin consensus by default (`--min-sources 3`)**: sharp consensus is a mean of per-source positional ranks, so a player covered by few sources has a noisier mean and lands further from ADP for reasons that are variance, not disagreement — roughly **5x** the divergence of fully-covered players, enough that they dominate the top of any list sorted by divergence alone. The report states how many it excluded; `--min-sources 0` (or `min_sources=0` on the MCP tool) opts out. **The measured figures live in one place only** — `reports/adp_divergence.py`, above `DEFAULT_MIN_SOURCES`, with the command to recompute them. They move with every board refresh, so don't restate digits here; three copies drifted apart once already.
...
- **`~/.fantasy-data/` holds a second DB this repo does not own**: `calls.db`, the append-only
  Calls ledger written solely by `quant-edge-mcp`'s `record_call`. It shares the directory, not
  the database — `fantasy_data.db` still has exactly one writer (this repo). Don't read or write
  `calls.db` from here; if fantasy-data ever needs the calls, join them MCP-side or expose a
  report the server calls.
- **Backups**: `scripts/backup_db.sh [dest_dir]` (default `~/.fantasy-data/backups`) snapshots via SQLite's `VACUUM INTO`, not `cp` — the engine runs in WAL mode, so a plain copy can miss commits still sitting in the `-wal` file or land mid-write. Run it before anything destructive (a season rollover, a schema change). No retention policy; prune old snapshots by hand.
